 ...
 * a una representaci� gr�fica. Creat per Marc Albareda
 *
 */
...
public class Taulell extends JPanel {
	...
	private int PAD = 20; // el Padding es el marge que tindr� el taulell.
	private int FILES = -1;
	private int COLS = -1;
	private int[][] a;
	...
	private String[] lletres = { "0", "1", "2", "3", "4", "5", "6", "7", "8", "*" }; // qu� s'ha d'escriure en cada
	...
	/**
	 * M�todes de dibuix. Toqueu-ho sota el vostre risc.
	 */

	// M�tode principal de dibuix. Cada vegada que es crida el "repaint()" es va
	// aqu�.
	...
	// Aquest m�tode inicialitza la matriu per primera vegada creant els quadrats
	private void initSquares() {

		squares = new SquareRx2[FILES][COLS];
		int w = getWidth();
		int h = getHeight();
		double xInc = (double) (w - 2 * PAD) / COLS; // aixo es geometria eh. Basicament diu que cada cuadrat tindr� de
	...
	private void initoverSquares(int fil, int col) {
		...
		int w = getWidth();
		int h = getHeight();
		double xInc = (double) (w - 2 * PAD) / col; // aixo es geometria eh. Basicament diu que cada cuadrat tindr� de
													// mida horitzontal la amplada total - els dos marges, dividit
													// per el nombre de quadrats que tens.
	...
	// aquest m�tode redibuixa el taulell si la finestra es canvia de mida.
	...
	/**
	 * M�tode de dibuix p�blic. Aquest m�tode rep la matriu i crida als altres
	 * m�todes.
	 */
...
class SquareRx2 {
	...
	public void mouseClick() {
		...
		t.setMousefil(fil);
		t.setMousecol(col);
		// si vols que les teves opcions vagin per ratol�, a partir d'aquesta funci�
		// hauries de cridar una funcio estatica de la TEVA classe. Tamb� pots consultar
		// les variables mitjan�ant els getters de mosuefil y mousecol
	}
}